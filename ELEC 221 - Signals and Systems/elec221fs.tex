...
\begin{tabular}{lll}  
	...
	Differentiation ($t$) & $\ds \frac{d^nx}{dt^n}$& $(j\omega)^n X(\omega)$\\
	...
	Multiplication & $x(t)y(t)$ & $\frac{1}{2\pi}[X * Y](\omega)$\\
...
\begin{tabular}{lll}  
	...
	\midrule
	Time Shifting (causal) & $x[n-N]$, $n\in \mathbb{Z}$ & $z^{-N}X(z)$\\
	Time Shifting (acausal) & $x[n-N]$ & $z^{-N}X(z) + x[-1]z^{-N + 1}$\\
	& & $+x[-2]z^{-N+2} + \cdots x[-N]$\\
	Time reversal & $x[-n]$ & $X(z^{-1})$ \\
	Multiplication by $n$ & $nx[n]$ & $\ds -z\dv{X(z)}{z}$\\
	Multiplication by $n^2$ & $n^2x[n]$ & $\ds z^2 \frac{d^2X(z)}{dz^2} + z\dv{X(z)}{z}$\\
	Finite Difference & $x[n]-x[n-1]$ & $(1-z^{-1})X(z) - x[-1]$\\
	Accumulation & $\sum_{k=0}^{n}x[k]$ & $\ds \frac{X(z)}{1-z^{-1}}$\\
	\bottomrule
\end{tabular}
\vspace{1cm}
...
\tabm $\cos\theta = 1/2(e^{j\theta}+e^{-j\theta})$\\
\tabm $\sin\theta = 1/2j(e^{j\theta}-e^{-j\theta})$
\end{minipage}%
...
\begin{tabular}{ll}  
	...
	$\alpha$ & $2\pi\alpha\delta(\omega)$\\  
	$e^{-a\abs{t}}$, $a > 0$ & $\ds \frac{2a}{a^2 + \omega^2}$\\
	$\cos(\omega_0 t)$ & $\pi[\delta(\omega - \omega_0)] + \delta(\omega + \omega_0)$\\
	$\sin(\omega_0 t)$ & $-j\pi[\delta(\omega - \omega_0)] - \delta(\omega + \omega_0)$\\
	$\ds \frac{\sin(\omega t)}{\pi t}$ &  $u(\omega + \omega_0) - u(\omega - \omega_0)$\\
	$u(\omega + \tau) - u(\omega - \tau)$ & $\ds 2\tau \frac{\sin(\omega\tau)}{\omega\tau}$\\
	$\rect(t/T)$ & $T \sinc(T\omega/2)$\\
	\bottomrule
\end{tabular}
\vspace{0.7cm}
\subsection{One-sided Z-Transforms}
\begin{tabular}{ll}  
	\toprule
	$f[n]$ & $F(z)$\\
	\midrule
	$\delta[n]$ & 1, Whole z-plane\\
	$u[n]$ & $\ds \frac{1}{1-z^{-1}}$, $\abs{z} > 1$\\
	$nu[n]$ & $\ds \frac{z^{-1}}{(1-z^{-1})^2}$, $\abs{z} > 1$\\
	$n^2u[n]$ & $\ds \frac{z^{-1}(1+z^{-1})}{(1-z^{-1})^3}$, $\abs{z} > 1$\\
	$\alpha^n u[n]$, $\abs{\alpha} < 1$ & $\ds \frac{1}{1-\alpha z^{-1}}$, $\abs{z} > \abs{\alpha}$\\
	$n\alpha^nu[n]$, $\abs{\alpha} < 1$ & $\ds \frac{\alpha z^{-1}}{(1-\alpha z^{-1})^2}$, $\abs{z} > \abs{\alpha}$\\
	$\cos(\omega_0 n)u[n]$ & $\ds \frac{1-\cos(\omega_0)z^{-1}}{1-2\cos(\omega_0)z^{-1} + z^{-2}}$, $\abs{z} > 1$\\
	$\sin(\omega_0 n)u[n]$ & $\ds \frac{\sin(\omega_0)z^{-1}}{1-2\cos(\omega_0)z^{-1} + z^{-2}}$, $\abs{z} > 1$\\
	\bottomrule
\end{tabular}
\end{minipage}
