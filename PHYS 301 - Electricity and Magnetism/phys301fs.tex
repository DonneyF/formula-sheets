...
Continuity Equation:\\
\tab $\ds \grad \cdot \vb{J} = - \pdv{\rho}{t}$
...
Vector Potential when $\vb{J} \rightarrow \vb{0}$ at infinity:\\
...
Bound Currents:\\
\tab $\vb{J}_B = \curl{\vb{M}}$ \qquad $\vb{K}_B = \vb{M}\cross\vb{\hat{n}}$
...
\begin{itemize}
	\itemsep0em
	\item $B_\text{above}^\parallel - B_\text{below}^\parallel = \mu_0K$
